'use client';
// ═══════════════════════════════════════════════════════════════════
// CMD-Screening — UI Component
// ═══════════════════════════════════════════════════════════════════

import React from 'react';
import { Check, X, Minus, AlertTriangle, Info, CheckCircle2 } from 'lucide-react';
import { FIELDS, SECTIONS, evaluate } from './form';

function TripleState({ value, recent }) {
  const isFresh = recent;
  const base = 'inline-flex items-center justify-center w-8 h-8 rounded-md text-xs font-bold transition-all';
  if (value === true) {
    return <span className={`${base} bg-amber-100 text-amber-800 border-2 border-amber-300 ${isFresh ? 'ring-2 ring-amber-400 ring-offset-2' : ''}`}>JA</span>;
  }
  if (value === false) {
    return <span className={`${base} bg-emerald-100 text-emerald-800 border-2 border-emerald-300 ${isFresh ? 'ring-2 ring-emerald-400 ring-offset-2' : ''}`}>NEIN</span>;
  }
  ...
  return (
    <div className={`flex items-start gap-3 p-3 rounded-lg transition-colors ${isRecent ? 'bg-cyan-50' : 'hover:bg-slate-50'}`}>
      ...
      <div className="flex-1 min-w-0">
        <div className="text-sm text-slate-800 leading-snug">{field.label}</div>
...
function ResultBanner({ result }) {
  ...
  const Icons = { high: AlertTriangle, medium: AlertTriangle, low: CheckCircle2, info: Info };
  const Icon = Icons[result.severity] || Info;
  return (
    <div className={`rounded-lg border-2 p-4 flex items-start gap-3 ${styles[result.severity]}`}>
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 space-y-6">
      ...
      <div className="text-[11px] text-slate-500 leading-relaxed border-t border-slate-200 pt-4">
        <p className="mb-1">
          <strong>Indikation:</strong> Vor restaurativer und/oder kieferorthopädischer Behandlungsplanung
          und zur Abgrenzung unklarer Kiefer- und Gesichtsschmerzen und/oder orofazialer Dysfunktionen.
        </p>
        <p>
          <strong>Hinweis Palpation:</strong> Referenzmuskel M. masseter pars superficialis nach DC/TMD-Protokoll
          mit ca. 1,0 kg Druck. Kiefergelenk prä- oder intraauriculär palpieren.
        </p>
      </div>
    </div>
  );
